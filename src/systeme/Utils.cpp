...
// Utils.cpp (Debug)
//
// Ce fichier implemente une couche de debug "thread-safe" pour ESP32 :
// - Les traces Debug::print* sont poussees dans une queue FreeRTOS.
// - Une tache de fond vide la queue et ecrit sur Serial, ce qui evite que
//   plusieurs taches melangent leurs sorties (interleaving).
// - Option : un journal en RAM (memory log) peut etre active pour exposer
//   les logs via une API HTTP (utile en production).
//
// Note : on garde ce module plutot generique car il peut etre reutilise
// dans d'autres firmwares.
// -----------------------------------------------------------------------------

// ===================== Configuration interne =====================

// Taille max d'un message en caracteres (hors '\n' optionnel).
#ifndef DBG_LINE_MAX
#define DBG_LINE_MAX        1024
#endif

// Profondeur de la queue (stocke des pointeurs DebugMsg*).
#ifndef DBG_QUEUE_DEPTH
#define DBG_QUEUE_DEPTH     1024
#endif

// Taille max pour un "groupe" (burst atomique) (buffer statique, pas de realloc).
...
// Envoie le buffer "groupe" vers la queue par morceaux (<= DBG_LINE_MAX),
// avec '\n' optionnel a la fin.
